...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create) {
    ...
    return pgtable_entry = pgtable + PTX(va);
...
void page_init(void) {
    ...
    for (; i < npage; i++) {
        pages[i].pp_ref = 0;
        /*** Exercise 3.11 ***/
        if (page2kva(pages + i) == TIMESTACK - BY2PG) continue;
...
int page_alloc(struct Page **pp) {
     ...
     * Hint: use `bzero`. */
    /**page2kva --- Get the kernel virtual address of Page 'pp'.(pmap.h)*/
    /**bzero --- clear mem(use virtual address). (init.c)*/
...
int page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm) {
    ...
    //check whether `va` is already mapping to `pa`
